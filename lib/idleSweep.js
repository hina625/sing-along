...
export async function runIdleSweep() {
    ...
    for (const r of candidates) {
        ...
        // Diagnostic: shows how idle each live room is and whether it's resetting.
        console.log(
            `[idle-sweep] room=${r.room_id} idleMin=${idleMin.toFixed(1)} ` +
            `warn=${warnMin} end=${endMin} warned=${r.idleWarnedAt ? 'yes' : 'no'} ` +
